...
document.addEventListener("DOMContentLoaded", (event) => {
    ...
    hbAudio.volume = 0.6; // głośność bicia serca
    ...
    const initBgMusic = () => {
        if (!bgMusicStarted && bgMusic) {
            bgMusicStarted = true;
            bgMusic.volume = 0.04; // bardzo delikatnie słyszalna
    ...
    if (cake) {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            aud.volume = 0.08; // Ściszone zgodnie z prośbą (ok. 8%)
            aud.preload = 'auto';
        ...
        cake.addEventListener('pointerdown', (e) => {
            ...
            if (clickCount >= REQUIRED_CLICKS) {
                ...
                if (audio) {
                    audio.volume = 0.08; // Cichsze (8%), by służyło jako delikatne tło dźwiękowe
                    audio.currentTime = 0;
...
function triggerFireworks() {
    ...
    if (audio) {
        audio.volume = 0.18; // Ściszone fajerwerki (18%)
        audio.currentTime = 0;
